...
import os
import pdb
dataset_name = "gdrive/My Drive/ML-Progetto/dataset/sport/sport" 
...
files_input = ["%s-val.csv"%dataset_name,"%s-train.csv"%dataset_name, "%s-test.csv"%dataset_name]
...
for (filename_input, filename_output) in zip(files_input, files_output):
    ...
    for i in range(len(folders)):
        ...
        dir_files = os.listdir(os.path.join("%sframe-video"%ROOT_FOLDER, curFolder))
        output.append("%s %d %d"%(curFolder, int(len(dir_files)), int(curIDX)))
